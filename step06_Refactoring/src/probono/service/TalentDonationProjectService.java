...
public class TalentDonationProjectService {
	...
	public void donationProjectUpdate(String projectName, Donator people) throws Exception {
		...
		*/
		...
		if (people != null) {
			donationProjectList.stream().map(p -> {
				if (p == null)
					throw new IllegalArgumentException("프로젝트 이름과 기부자 정보 재 확인 하세요");
				return p;
			}).filter(p -> p.getTalentDonationProjectName().equals(projectName))
				.findFirst()
				.ifPresent(v -> v.setProjectDonator(people));
	...
	public void beneficiaryProjectUpdate(String projectName, Beneficiary people) {
		...
		/**
		 * 조건식에 맞는 project 필터링 -> 가장 첫 번째 항목을 찾아 setProjectBeneficiary(people) 수행하고 종료.
		 */
